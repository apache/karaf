...
public class NamespaceHandler implements org.apache.aries.blueprint.NamespaceHandler {
    ...
    public URL getSchemaLocation(String namespace) {
        switch (namespace) {
            case "http://karaf.apache.org/xmlns/jasypt/v1.0.0":
                return getClass()
                        .getResource(
                                "/org/apache/karaf/jaas/blueprint/jasypt/handler/karaf-jasypt-1.0.0.xsd");
            default:
                return null;
        }
    }
    ...
    public ComponentMetadata decorate(
            Node node, ComponentMetadata componentMetadata, ParserContext parserContext) {
                    ...
                    if (ENCRYPTOR_ELEMENT.equals(name)) {
                        if (encryptorRef != null) {
                            throw new ComponentDefinitionException(
                                    "Only one of "
                                            + ENCRYPTOR_REF_ATTRIBUTE
                                            + " attribute or "
                                            + ENCRYPTOR_ELEMENT
                                            + " element is allowed");
                        }
                        BeanMetadata encryptor =
                                context.parseElement(BeanMetadata.class, metadata, e);
