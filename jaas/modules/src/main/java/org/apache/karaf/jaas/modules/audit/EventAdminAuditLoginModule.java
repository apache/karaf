...
public class EventAdminAuditLoginModule extends AbstractAuditLoginModule {
    ...
    protected void audit(Action action, String user) {
        try {
            EventAdminAuditor.audit(
                    bundleContext, topic + action.toString().toUpperCase(), user, subject);
        } catch (Throwable t) {
            if (!errorLogged) {
                errorLogged = true;
    ...
    static class EventAdminAuditor {
        public static void audit(
                BundleContext bundleContext, String topic, String username, Subject subject) {
