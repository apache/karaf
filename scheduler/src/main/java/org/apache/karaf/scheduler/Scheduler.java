...
/**
 * A scheduler to schedule time/cron based jobs. A job is an object that is executed/fired by the
 * scheduler. The object should either implement the {@link Job} interface or the {@link Runnable}
 * interface.
 *
 * <p>A job can be scheduled either by creating a {@link ScheduleOptions} instance through one of
 * the scheduler methods and then calling {@link #schedule(Object, ScheduleOptions)} or by using the
 * whiteboard pattern and registering a Runnable service with either the {@link
 * #PROPERTY_SCHEDULER_EXPRESSION} or {@link #PROPERTY_SCHEDULER_PERIOD} property. Services
 * registered by the whiteboard pattern can by default run concurrently, which usually is not
 * wanted. Therefore it is advisable to also set the {@link #PROPERTY_SCHEDULER_CONCURRENT} property
 * with Boolean.FALSE.
 */
public interface Scheduler {

    /**
     * Name of the configuration property to define the period for a job. The period is expressed in
     * seconds. This property needs to be of type Long.
     */
     ...
     * immediate. Default is to not startup immediate, the job is started the first time after the
     * period has expired. This property needs to be of type Boolean.
     */
     ...
     *
     * <p>Note that if a job with the same name has already been added, the old job is cancelled and
     * this new job replaces the old job.
     *
     * <p>The job object needs either to be a {@link Job} or a {@link Runnable}. The options have to
     * be created by one of the provided methods from this scheduler.
     *
     ...
     * @see #EXPR(String)
     */
    void schedule(Object job, ScheduleOptions options)
     ...
     */
    void reschedule(String jobName, ScheduleOptions options)
            throws IllegalArgumentException, SchedulerError;
     ...
     * @return <code>True</code> if the job existed and could be stopped, <code>false</code>
     *     otherwise.
     */
     ...
     *
     * @param times The number of times this job should be started (must be higher than 1 or -1 for
     *     endless).
