...
public class BootFeaturesInstallerTest extends TestBase {
    ...
    public void testParser() {
        BootFeaturesInstaller installer =
                new BootFeaturesInstaller(null, null, new String[0], "", false);
        Assert.assertEquals(
                asList(setOf("test1", "test2"), setOf("test3")),
                installer.parseBootFeatures(" ( test1 , test2 ) , test3 "));
        Assert.assertEquals(
                asList(setOf("test1", "test2", "test3")),
                installer.parseBootFeatures(" test1 , test2, test3"));
        Assert.assertEquals(
                asList(setOf("test1"), setOf("test2"), setOf("test3")),
                installer.parseBootFeatures("(test1), (test2), test3"));
    }
    ...
    public void testDefaultBootFeatures() throws Exception {
        ...
        impl.installFeatures(
                capture(featuresCapture), eq(EnumSet.of(Option.NoFailOnFeatureNotFound)));
        expectLastCall();
        ...
        replay(impl);
        BootFeaturesInstaller bootFeatures =
                new BootFeaturesInstaller(
                        null, impl, new String[0], "config,standard,region", false);
    ...
    public void testStagedBoot() throws Exception {
        ...
        replay(impl);
        BootFeaturesInstaller bootFeatures =
                new BootFeaturesInstaller(null, impl, new String[0], "(transaction), ssh", false);
        bootFeatures.installBootFeatures();
        verify(impl);
    }
    ...
    public void testStartDoesNotFailWithOneInvalidUri() throws Exception {
        ...
        expectLastCall()
                .andThrow(new IllegalArgumentException("Part of the test. Can be ignored."));
        ...
        BootFeaturesInstaller bootFeatures =
                new BootFeaturesInstaller(null, impl, repositories, "", false);
    ...
    public void testParseBootFeatures() throws Exception {
        String features =
                "foo, jim, (ssh, shell, jaas, feature, framework), (system, bundle, management, service), (instance, package, log, deployer, diagnostic, config, kar), bar, zad";
        BootFeaturesInstaller bootFeatures =
                new BootFeaturesInstaller(null, null, null, null, false);
        ...
        for (String f :
                Arrays.asList(
                        "instance", "package", "log", "deployer", "diagnostic", "config", "kar")) {
