...
public class BlacklistTest {
    ...
    public void testBlacklistFeatureWithRange() throws IOException {
        ...
        assertTrue(
                features.noneMatch(
                        f -> f.getId().equals("spring/2.5.6.SEC02") && !f.isBlacklisted()));
    }
    ...
    public void testBlacklistBundle() throws IOException {
        String blacklisted =
                "mvn:org.apache.servicemix.bundles/org.apache.servicemix.bundles.jasypt/1.7_1";
        Stream<Feature> features = blacklistWith(blacklisted);
        Stream<BundleInfo> bundles = features.flatMap(f -> f.getBundles().stream());
        assertTrue(
                bundles.noneMatch(b -> b.getLocation().equals(blacklisted) && !b.isBlacklisted()));
    }

    @Test
    public void testBlacklistLoad() throws URISyntaxException {
        Blacklist blacklist =
                new Blacklist(getClass().getResource("blacklist.txt").toExternalForm());
    ...
    private Stream<org.apache.karaf.features.Feature> blacklistWith(String blacklistClause)
        ...
        File blacklistedProperties =
                File.createTempFile("blacklisted-", ".properties", new File("target"));
        ...
        FeaturesServiceConfig config =
                new FeaturesServiceConfig(
                        ...
                        0,
                        0,
                        blacklistedProperties.toURI().toString(),
                        null,
                        null,
                        null);
