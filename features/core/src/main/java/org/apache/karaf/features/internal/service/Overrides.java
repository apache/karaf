...
public final class Overrides {
     ...
     *
     * <p>The file containing the overrides will be loaded from the given url. Blank lines and lines
     * starting with a '#' will be ignored, all other lines are considered as urls to override
     * bundles.
     *
     * <p>The list of resources to resolve will be scanned and for each bundle, if a bundle override
     * matches that resource, it will be used instead.
     *
     * <p>Matching is done on bundle symbolic name (they have to be the same) and version (the
     * bundle override version needs to be greater than the resource to be resolved, and less than
     * the next minor version. A range directive can be added to the override url in which case, the
     * matching will succeed if the resource to be resolved is within the given range.
     *
     ...
     */
    @Deprecated
    public static <T extends Resource> void override(
            Map<String, T> resources, Collection<String> overrides) {
        // Do override replacement
        for (Clause override :
                Parser.parseClauses(overrides.toArray(new String[overrides.size()]))) {
    ...
    /**
     * Input map of resources is checked - if there are matching resources in <code>overridenFrom
     * </code> and there's <strong>no</strong> symbolic name matching, resource for original URI is
     * restored. Effectively this method reverts {@link
     * org.apache.karaf.features.internal.model.processing.BundleReplacements.BundleOverrideMode#MAVEN
     * maven} override mode if there's no symbolic name matching.
     *
     * <p>About versions - with previous <code>${karaf.etc}/overrides.properties</code> both
     * symbolic name should match <strong>and</strong> versions should be compatible - either using
     * implicit rules or by means of <code>range</code> clause. With new mechanism, we know we
     * should use OSGi or Maven override, but we loose information about OSGi version range matching
     * - we assume then that version rules were applied at features JAXB model processing time.
     *
     * @param resources
     * @param overridenFrom
     * @param <T>
     */
    public static <T extends Resource> void override(
            Map<String, T> resources, Map<String, T> overridenFrom) {
     ...
     */
    private static <T extends Resource> boolean shouldOverride(
            Resource resource, T override, String explicitRange) {
