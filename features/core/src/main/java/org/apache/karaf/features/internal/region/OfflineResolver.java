...
public class OfflineResolver {
    ...
    public static void resolve(String resolutionFile) throws Exception {
        ...
        try (BufferedReader reader =
                Files.newBufferedReader(Paths.get(resolutionFile), StandardCharsets.UTF_8)) {
                        ...
                        new ResolveContext() {
                            ...
                            public List<Capability> findProviders(Requirement requirement) {
                                ...
                                Collection<Capability> res =
                                        resMap != null ? resMap.get(requirement) : null;
                                if (res != null && !res.isEmpty()) {
                                    caps.addAll(res);
                                } else if (globalRepository != null) {
                                    // Only bring in external resources for non optional
                                    // requirements
                                    if (!RESOLUTION_OPTIONAL.equals(
                                            requirement
                                                    .getDirectives()
                                                    .get(RESOLUTION_DIRECTIVE))) {
                                        resMap =
                                                globalRepository.findProviders(
                                                        Collections.singleton(requirement));
