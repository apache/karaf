...
@Command(
        scope = "feature",
        name = "repo-remove",
        description = "Removes the specified repository features service.")
@Service
public class RepoRemoveCommand extends FeaturesCommandSupport {
    ...
    @Argument(
            index = 0,
            name = "repository",
            description = "Shortcut name of the feature repository or the full URI",
            required = true,
            multiValued = false)
    ...
    @Option(
            name = "-u",
            aliases = {"--uninstall-all"},
            description = "Uninstall all features from the repository",
            required = false,
            multiValued = false)
    ...
    protected void doExecute(FeaturesService featuresService) throws Exception {
        URI uri;
        {
            ...
            if (uris.size() > 1) {
                System.err.println(
                        "Multiple matching repositories for "
                                + nameOrUrl
                                + (version != null ? " / " + version : ""));
                return;
            }
