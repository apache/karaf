...
public abstract class AbstractFeatureMojo extends MojoSupport {
    ...
    @Parameter protected List<String> descriptors;

    protected Set<Artifact> descriptorArtifacts;

    @Parameter protected List<String> features;

    @Parameter protected boolean addTransitiveFeatures = true;

    @Parameter private boolean includeMvnBasedDescriptors = false;

    @Parameter private boolean failOnArtifactResolutionError = true;

    @Parameter private boolean resolveDefinedRepositoriesRecursively = true;

    @Parameter protected boolean skipNonMavenProtocols = true;
    ...
    @Parameter private int defaultStartLevel = 80;
    ...
    protected void retrieveDescriptorsRecursively(
            String uri, Set<String> bundles, Map<String, Feature> featuresMap) {
    ...
    /**
     * Resolves and copies the given artifact to the repository path. Prefers to resolve using the
     * repository of the artifact if present.
     *
    ...
    protected void resolveArtifact(Artifact artifact, List<ArtifactRepository> remoteRepos) {
        try {
            ...
            List<ArtifactRepository> usedRemoteRepos =
    ...
    /**
     * Populate the features by traversing the listed features and their dependencies if transitive
     * is true
     *
    ...
    protected void addFeatures(
        ...
        for (String feature : featureNames) {
            ...
            Feature f =
                    getMatchingFeature(featuresMap, split[0], split.length > 1 ? split[1] : null);
            features.add(f);
            if (transitive) {
    ...
    protected void addFeaturesDependencies(
            List<Dependency> featureNames,
            Set<Feature> features,
            Map<String, Feature> featuresMap,
            boolean transitive) {
        for (Dependency dependency : featureNames) {
            Feature f =
                    getMatchingFeature(featuresMap, dependency.getName(), dependency.getVersion());
            features.add(f);
            if (transitive) {
                addFeaturesDependencies(f.getFeature(), features, featuresMap, true);
            }
    ...
    private Feature getMatchingFeature(
            Map<String, Feature> featuresMap, String feature, String version) {
        Feature f = null;
                ...
                for (String key : featuresMap.keySet()) {
                    ...
                    if (feature.equals(nameVersion[0])) {
                        ...
                        if (versionRange.contains(ver)) {
                            if (f == null
                                    || VersionTable.getVersion(f.getVersion())
                                                    .compareTo(
                    ...
                    for (Conditional conditional : feature.getConditional()) {
                        for (BundleInfo bundle : conditional.getBundles()) {
                            if (ignoreDependencyFlag
                                    || (!ignoreDependencyFlag && !bundle.isDependency())) {
                                resolveArtifact(bundle.getLocation());
                            }
                        }
                    }
    ...
    /**
     * Maven ArtifactResolver leaves file handles around so need to clean up or we will run out of
     * file descriptors.
     */
