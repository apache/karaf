...
public abstract class ObrCommandSupport implements Action {
    ...
    protected Resource[] searchRepository(
    ...
    protected void doDeploy(
            RepositoryAdmin admin, List<String> bundles, boolean start, boolean deployOptional)
            throws Exception {
        ...
        if ((resolver.getAddedResources() != null) && (resolver.getAddedResources().length > 0)) {
            if (resolver.resolve(deployOptional ? 0 : Resolver.NO_OPTIONAL_RESOURCES)) {
                ...
                for (int resIdx = 0; (resources != null) && (resIdx < resources.length); resIdx++) {
                    System.out.println(
                            "   "
                                    + getResourceId(resources[resIdx])
                                    + " ("
    ...
    protected String getResourceId(Resource resource) {
        return resource.getPresentationName() != null
                ? resource.getPresentationName()
                : resource.getSymbolicName();
    }

    protected Requirement parseRequirement(RepositoryAdmin admin, String req)
            throws InvalidSyntaxException {
        int p = req.indexOf(':');
        String name;
        String filter;
    ...
    protected Requirement[] parseRequirements(RepositoryAdmin admin, List<String> requirements)
            throws InvalidSyntaxException {
    ...
    protected void persistRepositoryList(RepositoryAdmin admin) {
        try {
            ...
            try (BufferedWriter writer =
                    new BufferedWriter(new OutputStreamWriter(new FileOutputStream(sysTmp)))) {
                if (sys.exists()) {
                    try (BufferedReader reader =
                            new BufferedReader(new InputStreamReader(new FileInputStream(sys)))) {
