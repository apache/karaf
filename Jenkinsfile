...
pipeline {
    ...
    stages {
        ...
        stage('Deploy') {
            when {
                expression {
                    env.BRANCH_NAME ==~ /(4.1.x|4.2.x|master)/
                }
            }
            steps {
                echo 'Deploying'
    ...
    post {
        ...
        failure {
            script {
                if(env.BRANCH_NAME == "4.1.x" || env.BRANCH_NAME == "4.2.x" || env.BRANCH_NAME == "master") {
                    emailext(
...
""",
        ...
        success {
            ...
            deleteDir()
            script {
                if ((env.BRANCH_NAME == "4.1.x" || env.BRANCH_NAME == "4.2.x" || env.BRANCH_NAME == "master") && (currentBuild.previousBuild != null) && (currentBuild.previousBuild.result != 'SUCCESS')) {
                    emailext (
