...
public class EnvironmentDumpProvider extends TextDumpProvider {
    ...
    protected void writeDump(final OutputStreamWriter outputStream) throws Exception {
        ...
        outPW.printf(
                        KEY_VALUE_FORMAT,
                        "Dump timestamp",
                        dateTimeFormatInstance.format(new Date(System.currentTimeMillis())))
                .println();
        outPW.println();
        // karaf information
    ...
    private void dumpKarafInformation(final PrintWriter outPW) {
        outPW.printf(
                ...
                .println();
        outPW.printf(INDENT_KEY_VALUE_FORMAT, "home", System.getProperty("karaf.home", ""))
                .println();
        outPW.printf(INDENT_KEY_VALUE_FORMAT, "base", System.getProperty("karaf.base", ""))
                .println();
    }
    ...
    private void dumpOSInformation(final PrintWriter outPW) {
        ...
        outPW.printf(INDENT_KEY_VALUE_FORMAT, "processors", mxBean.getAvailableProcessors())
                .println();
        //        outPW.printf(INDENT_KEY_VALUE_FORMAT, "current system load average",
        // mxBean.getSystemLoadAverage()).println();
    }

    private void dumpVMInformation(
            final PrintWriter outPW, final DateFormat dateTimeFormatInstance) {
        ...
        outPW.printf(
                        KEY_VALUE_FORMAT,
                        "Start time",
                        dateTimeFormatInstance.format(new Date(mxBean.getStartTime())))
                .println();
        outPW.printf(KEY_VALUE_FORMAT, "Uptime", printDuration(mxBean.getUptime())).println();
        outPW.println();
        outPW.printf(KEY_VALUE_FORMAT, "Java VM", mxBean.getVmName() + " " + mxBean.getVmVersion())
                .println();
        outPW.printf(INDENT_KEY_VALUE_FORMAT, "vendor", mxBean.getVmVendor()).println();
        outPW.printf(INDENT_KEY_VALUE_FORMAT, "version", System.getProperty("java.version"))
                .println();
        outPW.println();
        ...
        outPW.printf(INDENT_KEY_VALUE_FORMAT, "boot classpath", mxBean.getBootClassPath())
                .println();
        ...
        if (compilationMXBean != null) {
            ...
            outPW.printf(
                            INDENT_KEY_VALUE_FORMAT,
                            "total compile time",
                            printDuration(compilationMXBean.getTotalCompilationTime()))
                    .println();
        }
    }
    ...
    private void dumpThreadsInformation(final PrintWriter outPW) {
        ...
        outPW.printf(INDENT_KEY_VALUE_FORMAT, "live", formatLong(mxBean.getThreadCount()))
                .println();
        outPW.printf(INDENT_KEY_VALUE_FORMAT, "daemon", formatLong(mxBean.getDaemonThreadCount()))
                .println();
        outPW.printf(INDENT_KEY_VALUE_FORMAT, "peak", formatLong(mxBean.getPeakThreadCount()))
                .println();
        outPW.printf(
                        INDENT_KEY_VALUE_FORMAT,
                        "total",
                        formatLong(mxBean.getTotalStartedThreadCount()))
                .println();
    }
    ...
    private void dumpClassesInformation(final PrintWriter outPW) {
        ...
        outPW.printf(INDENT_KEY_VALUE_FORMAT, "loaded", formatLong(mxBean.getLoadedClassCount()))
                .println();
        outPW.printf(
                        INDENT_KEY_VALUE_FORMAT,
                        "total",
                        formatLong(mxBean.getTotalLoadedClassCount()))
                .println();
        outPW.printf(
                        INDENT_KEY_VALUE_FORMAT,
                        "unloaded",
                        formatLong(mxBean.getUnloadedClassCount()))
                .println();
    }
    ...
    private void dumpMemoryInformation(final PrintWriter outPW) {
        ...
        if (heapMemoryUsage != null) {
            ...
            outPW.printf(
                            INDENT_KEY_VALUE_FORMAT,
                            "commited",
                            printMemory(heapMemoryUsage.getCommitted()))
                    .println();
            outPW.printf(INDENT_KEY_VALUE_FORMAT, "init", printMemory(heapMemoryUsage.getInit()))
                    .println();
            outPW.printf(INDENT_KEY_VALUE_FORMAT, "used", printMemory(heapMemoryUsage.getUsed()))
                    .println();
            outPW.printf(INDENT_KEY_VALUE_FORMAT, "maximal", printMemory(heapMemoryUsage.getMax()))
                    .println();
        }
        if (nonHeapMemoryUsage != null) {
            ...
            outPW.printf(
                            INDENT_KEY_VALUE_FORMAT,
                            "commited",
                            printMemory(nonHeapMemoryUsage.getCommitted()))
                    .println();
            outPW.printf(INDENT_KEY_VALUE_FORMAT, "init", printMemory(nonHeapMemoryUsage.getInit()))
                    .println();
            outPW.printf(INDENT_KEY_VALUE_FORMAT, "used", printMemory(nonHeapMemoryUsage.getUsed()))
                    .println();
            outPW.printf(
                            INDENT_KEY_VALUE_FORMAT,
                            "maximal",
                            printMemory(nonHeapMemoryUsage.getMax()))
                    .println();
        }
    }
    ...
    private void dumpGCInformation(final PrintWriter outPW) {
        ...
        if (memoryMxBean != null) {
            outPW.printf(
                            INDENT_KEY_VALUE_FORMAT,
                            "pending objects",
                            formatLong(memoryMxBean.getObjectPendingFinalizationCount()))
                    .println();
        }
        ...
        for (final GarbageCollectorMXBean mxBean : mxBeans) {
            ...
            outPW.printf(
                            ...
                            String.format(
                                    ...
                                    printDuration(mxBean.getCollectionTime())))
                    .println();
        }
    }

    private String formatLong(final long longValue) {
        final NumberFormat fmtI =
