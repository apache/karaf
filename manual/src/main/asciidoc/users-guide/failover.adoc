...
WARNING: Apache Karaf won't start if the JDBC driver is not present in the `lib/ext` folder.


WARNING: The `sample` database will be created automatically if it does not exist.

WARNING: If the connection to the database is lost, the master instance tries to gracefully shutdown, allowing a slave instance to
...
WARNING: The `karaf.lock.jdbc.url` property contains a JDBC URL which requires an active SID. It means that you must manually create the Oracle
...
WARNING: Using 'hot' standby means that the slave instances are running and bind some ports. So, if you use master and slave instances on the same machine, you have
...
See the http://karaf.apache.org/index/subprojects/cellar.html[Apache Karaf Cellar website] for details.
