...
WARNING: For security reason, this key is disabled. We encourage to create the keys pair per client and update the `etc/keys.properties` file.
...
Some applications require specific security providers to be available, such as http://www.bouncycastle.org[BouncyCastle].
