...
WARNING: The `jms:create` command doesn't install any feature or bundle providing the JMS connection factory classes (and dependencies).
...
WARNING: Depending of the JMS connection factory type, this command may not work.
...
WARNING: The `jms:consume` command just consumes (so removes) messages from a JMS queue. It doesn't display the messages.
