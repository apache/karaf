...
Apache Karaf natively provides a failover mechanism. It uses a kind of primary/secondary topology where one instance is active
...
In both cases, a container-level lock system allows bundles to be preloaded into the secondary Karaf instance in order to provide faster failover performance.

===== HA/failover (active/passive)

The Apache Karaf failover capability uses a lock system.

This container-level lock system allows bundles installed on the primary to be preloaded on the secondary, in order to provide faster failover performance.
...
When a first instance starts, if the lock is available, it takes the lock and become the primary.
If a second instance starts, it tries to acquire the lock. As the lock is already hold by the primary, the instance becomes
a secondary, in standby mode (not active). A secondary periodically check if the lock has been released or not.
...
The configuration of the lock system has to be defined in the `etc/system.properties` file, on each instance (primary/secondary):
...
The primary instance holds the lock by locking a table in the database. If the primary loses the lock, a waiting secondary
...
[NOTE]
====
If the connection to the database is lost, the primary instance tries to gracefully shutdown, allowing a secondary instance to
become the primary when the database is back. The former primary instance will require a manual restart.
====
...
Apache Karaf supports container-level locking. It allows bundles to be preloaded into the secondary instance.
Thanks to that, switching to a secondary instance is very fast as the secondary instance already contains all required bundles.
...
|1
|A 'cold' standby instance. Core bundles are not loaded into container. Secondaries will wait until lock acquired to start server.

|<50
|A 'hot' standby instance. Core bundles are loaded into the container. Secondaries will wait until lock acquired to start user level bundles. The console will be accessible for each secondary instance at this level.
...
[NOTE]
====
Using 'hot' standby means that the secondary instances are running and bound to some ports. So, if you use primary and secondary instances on the same machine, you have
to update the secondary configuration to bind the services (ssh, JMX, etc) on different port numbers.
====
