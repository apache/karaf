...
* `karaf.lock.jdbc.url` property contains the JDBC URL to the database (H2 in this example).
* `karaf.lock.jdbc.driver` property contains the class name of the JDBC driver to use (H2 in this example).
...
Apache Karaf supports H2 database for locking. The lock implementation class name to use is `org.apache.karaf.main.lock.H2JDBCLock`:
