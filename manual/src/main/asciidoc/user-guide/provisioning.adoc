...
Apache Karaf provides two resolvers, that you can select with the `resolver` property in the `etc/org.apache.karaf.features.cfg` file:
...
* default: the features and bundles are installed using the OSGi resolver, computing the requirements/capabilities. It supports
feature and service requirements, conditional features, and resource repositories.
* simple: the features and bundles are installed in the declaration order (dependency features first), without using the OSGi
resolver. It's faster and deterministic, but the requirements/capabilities are not resolved: feature and service requirements,
...
ecf=https://download.eclipse.org/rt/ecf/latest/site.p2/karaf-features.xml
