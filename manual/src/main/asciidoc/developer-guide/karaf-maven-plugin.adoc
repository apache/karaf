...
===== `karaf:features-generate-description`
...
===== `karaf:features-validate-description`
...
The `karaf:features-create-kar` goal assembles a KAR archive from a features XML descriptor file, normally generated in
the same project with the `karaf:features-generate-descriptor` goal.
...
This choice is so other resources such as legal files from the maven-remote-resources-plugin can be included under META-INF in the kar, without getting installed into Karaf.
