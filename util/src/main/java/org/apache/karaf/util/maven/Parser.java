...
public class Parser {
     ...
     * @throws MalformedURLException if provided path does not comply to expected syntax or an
    ...
    public static String pathFromMaven(String uri, String resolved) throws MalformedURLException {
        ...
        if (resolved != null) {
            ...
            if (idx >= 0) {
                String version =
                        resolved.substring(
                                idx + grp.length(), resolved.indexOf('/', idx + grp.length()));
            ...
            if (p[p.length - 1].charAt(artifactIdVersion.length()) == '-') {
                classifier =
                        p[p.length - 1].substring(
                                artifactIdVersion.length() + 1, p[p.length - 1].lastIndexOf('.'));
            } else {
                classifier = null;
            }
     ...
     */
    private void parseArtifactPart(final String part) throws MalformedURLException {
    ...
    public String getSnapshotVersion(
            final String version, final String timestamp, final String buildnumber) {
