...
/**
 * Security MBean. This MBean can be used to find out whether the currently logged user can access
 ...
 *
 * <p>This MBean does not actually invoke any operations on the given objects, it only checks
 * permissions.
 */
public interface JMXSecurityMBean {
     ...
     * {@link #CAN_INVOKE_RESULT_ROW_TYPE} entries. It has a composite key composed by the
     * "ObjectName" and "Method" columns.
     */
    ...
    /**
     * The columns contained in a {@link #CAN_INVOKE_RESULT_ROW_TYPE}. The data types for these
     * columns are as follows:
     *
     * <ul>
     *   <li>"ObjectName": {@link SimpleType#STRING}
     *   <li>"Method": {@link SimpleType#STRING}
     *   <li>"CanInvoke": {@link SimpleType#BOOLEAN}
     * </ul>
     */
     ...
     * @return {@code True} if there is at least one method on the MBean that the user can invoke,
     *     {@code false} else.
     ...
     * @return {@code True} if there is an overload of the specified method that the user can
     *     invoke, {@code false} else.
     * @throws Exception If the invocation check fails.
     */
     ...
     *     the given name if {@code null} is used for the arguments. There may still be certain
     *     values that the user does not have permissions to pass to the method.
     * @throws Exception If the invocation check fails.
     */
    boolean canInvoke(String objectName, String methodName, String[] argumentTypes)
            throws Exception;

    /**
     * Bulk operation to check whether the current user can access the requested MBeans or invoke
     * the requested methods.
     *
     * @param bulkQuery
     *     <p>a map of Object Name to requested operations. Operations can be specified with or
     *     without argument types. An operation without arguments matches any overloaded method with
     *     this name. If an empty list is provided for the operation names, a check is done whether
     *     the current user can invoke <em>any</em> operation on the MBean. Example:
     *     <pre>{@code
    ...
    // a member class is used to initialize final fields, as this needs to do some exception
    // handling...
    class SecurityMBeanOpenTypeInitializer {
        ...
        static {
            try {
                TABULAR_TYPE =
                        new TabularType(
                                "CanInvokeTabularType",
                                "Result of canInvoke() bulk operation",
                                ROW_TYPE,
