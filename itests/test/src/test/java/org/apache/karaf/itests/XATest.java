...
public class XATest extends BaseTest {
    ...
    private String awaitCondition(String command, String expected, String description, Principal... roles) throws Exception {
    ...
    public void test() throws Exception {
        ...
        System.out.println(" ");
        String dsList = awaitCondition("jdbc:ds-list", "OK", "H2 datasource to become available", roles);
        System.out.println(dsList);

        System.out.println("== Creating table in H2 ==");
        System.out.println(executeCommand("jdbc:execute h2 CREATE TABLE IF NOT EXISTS messages (id INTEGER GENERATED BY DEFAULT AS IDENTITY NOT NULL, message VARCHAR(1024) NOT NULL, CONSTRAINT primary_key PRIMARY KEY (id))", roles));

        awaitCondition("jdbc:query h2 select * from messages", "MESSAGE", "table creation", roles);
