...
@Command(
        scope = "instance",
        name = "connect",
        description = "Connects to an existing container instance.")
@Service
public class ConnectCommand extends InstanceCommandSupport {
    ...
    @Option(
            name = "-k",
            aliases = {"--keyfile"},
            description = "Remote key file to use for key authentication",
            required = false,
            multiValued = false)
    ...
    protected Object doExecute() throws Exception {
        ...
        if (username != null) {
            if (password == null) {
                if (keyFile == null) {
                    session.execute(
                            "ssh:ssh -q -l " + username + " -p " + port + " localhost " + cmdStr);
                } else {
                    session.execute(
                            "ssh:ssh -q -l "
                                    + username
            ...
            if (keyFile == null) {
                ...
                session.execute(
                        "ssh:ssh -q -p " + port + " -k " + keyFile + " localhost " + cmdStr);
            }
        }
        return null;
    }
