...
/**
 * This class is used to create the sql statements for the Karaf lock tables that are used for
 * clustering of Karaf instances.
 *
 * <p>It will generate sql statement to create two separate tables, a lock table and a lock id
 * table: <code>
...
public class GenericStatements {
    ...
    /**
     * This method will return the name of the cluster that the instances are using to compete for
     * the master lock.
     *
     ...
     * generate the following sql statement. <code>
    ...
    private String getLockTableInitialInsertStatement() {
        return "INSERT INTO "
                + this.getLockTableName()
                + "(ID, STATE, LOCK_DELAY) VALUES (0, 0, 0)";
    }

    /**
     * This will be called when trying to acquire the lock and will generate the following sql
     * statement. <code>
     *  UPDATE KARAF_LOCK SET ID = ?, STATE = ?, LOCK_DELAY = ? WHERE ID = 0 OR ID = ?
     * </code> You are then expected to assign the values associated with the sql statement.
     *
     * @param id The new ID.
    ...
    public String getLockUpdateIdStatement(int id, int state, int lock_delay, int curId) {
        return String.format(
                "UPDATE %s SET ID = %d, STATE = %d, LOCK_DELAY = %d WHERE ID = 0 OR ID = %d",
                this.getLockTableName(), id, state, lock_delay, curId);
    }

    /**
     * This will be called when trying to steal the lock and will generate the following sql
     * statement. <code>
     *  UPDATE KARAF_LOCK SET ID = ?, STATE = ?, LOCK_DELAY = ? WHERE ( ID = 0 OR ID = ? ) AND STATE = ?
     * </code> You are then responsible to assign the values of the different fields using standard
     * JDBC statement calls.
     *
     * @param id The new ID.
     * @param state The new lock state.
     * @param lock_delay The new lock delay.
     ...
     * @return The SQL update statement.
     */
    public String getLockUpdateIdStatementToStealLock(
            int id, int state, int lock_delay, int curId, int curState) {
        return String.format(
                "UPDATE %s SET ID = %d, STATE = %d, LOCK_DELAY = %d WHERE ( ID = 0 OR ID = %d ) AND STATE = %d",
     ...
     * statement.
     *
     * <p>UPDATE KARAF_LOCK SET ID = 0 WHERE ID = ?
     *
    ...
    /**
     * This method will generate the create table sql statement to create the karaf id table and
     * will generate the following sql statement. <code>
    ...
    public String getLockIdUpdateIdStatement(int id, int curId) {
        return String.format(
                "UPDATE %s SET ID = %d WHERE ID = %d", this.getLockIdTableName(), id, curId);
    }
