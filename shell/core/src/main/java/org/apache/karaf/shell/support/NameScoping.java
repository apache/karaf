...
public class NameScoping {
    ...
    /**
     * Return the name of the command which can omit the global scope prefix if the command starts
     * with the same prefix as the current application.
     *
    ...
    /**
     * Return true if the given scope is the global scope so that it can be hidden from help
     * messages.
     *
     * @param session the command session.
    ...
    /**
     * Return true if we are in multi-scope mode (the default) or if we are in single scope mode
     * which means we avoid prefixing commands with their scope.
     *
     * @param session the command session.
