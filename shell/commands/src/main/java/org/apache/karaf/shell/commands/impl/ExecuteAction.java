...
public class ExecuteAction implements Action {
    ...
    @Argument(
            index = 0,
            name = "command",
            description = "Execution command with arguments",
            required = true,
            multiValued = true)
    ...
    public Object execute() throws Exception {
        ProcessBuilder builder = new ProcessBuilder(args).directory(session.currentDir().toFile());
        ...
        PumpStreamHandler handler =
                new PumpStreamHandler(cp.in(), cp.out(), cp.err(), "Command '" + cmd + "'");
