...
public class WcAction extends AbstractAction {
    ...
    protected Object doExecute() throws Exception {
        ...
        if (files == null) {
            ...
            resultString = getFilesWordCounts(files);
        }
    ...
    protected void setDefaultOptions() {
        boolean noOptionSupplied = !(lines || words || chars || bytes);
     ...
     */
    protected String getFilesWordCounts(List<File> files) throws IOException {
        StringBuilder stringBuilder = new StringBuilder();
        WordCounts totalWordCounts = new WordCounts();
        for (File file : files) {
            WordCounts fileWordCounts = getWordCounts(new FileInputStream(file));
            String fileResultString = formatWordCounts(fileWordCounts, file.getName());

            totalWordCounts = totalWordCounts.add(fileWordCounts);
