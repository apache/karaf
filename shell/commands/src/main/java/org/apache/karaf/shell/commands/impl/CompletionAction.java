...
@Command(
        scope = "shell",
        name = "completion",
        description = "Display or change the completion mode on the current console session.")
@Service
    ...
    public Object execute() throws Exception {
        if (mode == null) {
            System.out.println(session.get(Session.COMPLETION_MODE));
                ...
                && !mode.equalsIgnoreCase("first")
                && !mode.equalsIgnoreCase("subshell")) {
            System.err.println(
                    "The completion mode is not correct. The valid modes are: global, first, subshell. See documentation for details.");
        } else {
