...
@Command(
        scope = "wrapper",
        name = "install",
        description = "Install the container as a system service in the OS.")
@Service
public class Install implements Action {
    ...
    @Reference private WrapperService wrapperService;

    @Override
    public Object execute() throws Exception {
        File[] wrapperPaths =
                wrapperService.install(name, displayName, description, startType, envs, includes);
        ...
        if (os.startsWith("Win")) {
            ...
            if (redhatRelease.exists()) {
                ...
                System.out.println(
                        INTENSITY_BOLD
                                + "On Redhat/Fedora/CentOS Systems (SystemV):"
                                + INTENSITY_NORMAL);
                System.out.println("  To install the service:");
                System.out.println("    $ ln -s " + serviceFile.getPath() + " /etc/init.d/");
                System.out.println("    $ chkconfig " + serviceFile.getName() + " --add");
                ...
                System.out.println("");
                System.out.println(
                        INTENSITY_BOLD + "On Ubuntu/Debian Systems (SystemV):" + INTENSITY_NORMAL);
                System.out.println("  To install the service:");
                System.out.println("    $ ln -s " + serviceFile.getPath() + " /etc/init.d/");
                System.out.println("");
                System.out.println("  To start the service when the machine is rebooted:");
            ...
            if (systemdFile != null) {
                System.out.println("");
                System.out.println(
                        INTENSITY_BOLD + "For systemd compliant Linux: " + INTENSITY_NORMAL);
            ...
            System.out.println("");
            System.out.println(
                    INTENSITY_BOLD + "Solaris/SunOS system detected :" + INTENSITY_NORMAL);
            System.out.println("  To install the service (and enable at system boot):");
            System.out.println("    $ ln -s " + serviceFile.getPath() + " /etc/init.d/");
            System.out.println("");
            System.out.println(
                    "  To start the service when the machine is rebooted for all multi-user run levels");
            ...
            System.out.println("");
            System.out.println(
                    "    If your application makes use of other services, then you will need to make");
            System.out.println(
                    "    sure that your application is started after, and then shutdown before. This");
            System.out.println(
                    "    is done by controlling the startup/shutdown order by setting the right order");
            ...
            System.out.println("  To install the service (and enable at system boot):");
            System.out.println(
                    "    $ cp /sbin/init.d/template /sbin/init.d/" + serviceFile.getName());
            System.out.println(
                    "    $ cat /sbin/init.d/"
                            + serviceFile.getName()
                            + " | sed 's/<specific>/"
                            ...
                            + "/g' | \\ ");
            System.out.println(
                    "      awk '/# Execute the commands to/{print;print \"                set_return\";next}1' | \\ ");
            System.out.println("      sed 's/CONTROL_VARIABLE/CONTROL_VARIABLE_KARAF/g' | \\ ");
            System.out.println(
                    "      sed 's@# Execute the commands to start your subsystem@        "
                            + serviceFile.getPath()
                            + " start@g' | \\ ");
            System.out.println(
                    "      sed 's@# Execute the commands to stop your subsystem@        "
                            + serviceFile.getPath()
                            + " stop@g' | \\ ");
            System.out.println(
                    "      sed 's/^[ \t]*.://g' > /sbin/init.d/" + serviceFile.getName() + ".tmp");
            System.out.println(
                    "    $ mv /sbin/init.d/"
                            + serviceFile.getName()
                            + ".tmp /sbin/init.d/"
            ...
            System.out.println(
                    "    $ echo CONTROL_VARIABLE_KARAF=1 > /etc/rc.config.d/"
                            + serviceFile.getName());
            System.out.println("");
            System.out.println(
                    "  To start the service when the machine is rebooted for multi-user run level");
            System.out.println("  and stopped for the halt, single-user and reboot runlevels:");
            System.out.println(
                    "    $ ln -s /sbin/init.d/"
                            + serviceFile.getName()
