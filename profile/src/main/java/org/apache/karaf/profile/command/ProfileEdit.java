...
@Command(
        name = "edit",
        scope = "profile",
        description = "Edits the specified profile",
        detailedDescription = "classpath:profileEdit.txt")
@Service
public class ProfileEdit implements Action {
    ...
    @Option(
            name = "-i",
            aliases = {"--import-pid"},
            description = "Imports the pids that are edited, from local OSGi config admin",
            required = false,
            multiValued = false)
    ...
    @Option(
            name = "--delimiter",
            description = "Specifies the delimiter to use for appends and removals.")
    private String delimiter = ",";

    @Argument(
            index = 0,
            name = "profile",
            description = "The target profile to edit",
            required = true,
            multiValued = false)
    ...
    private void editProfile(Profile profile) throws Exception {
        ...
        if (endorsed != null && endorsed.length > 0) {
            editInLine = true;
            handleLibraries(
                    builder, endorsed, profile, "endorsed lib", ProfileConstants.ENDORSED_PREFIX);
        }
        if (extension != null && extension.length > 0) {
            editInLine = true;
            handleLibraries(
                    builder, extension, profile, "extension lib", ProfileConstants.EXT_PREFIX);
        }
        ...
        if (!editInLine) {
            ...
            // If a single pid has been selected, but not a key value has been specified or import
            // has
            // been selected,
    ...
    private void handleFeatures(ProfileBuilder builder, String[] features, Profile profile) {
        ...
        for (String feature : features) {
            if (delete) {
                System.out.println(
                        "Deleting feature:" + feature + " from profile:" + profile.getId());
            } else {
                System.out.println("Adding feature:" + feature + " to profile:" + profile.getId());
            }
            updateConfig(
                    conf,
                    ProfileConstants.FEATURE_PREFIX + feature.replace('/', '_'),
                    feature,
                    set,
                    delete);
    ...
    private void handleFeatureRepositories(
            ProfileBuilder builder, String[] repositories, Profile profile) {
        Map<String, Object> conf = getConfigurationFromBuilder(builder, Profile.INTERNAL_PID);
        for (String repositoryURI : repositories) {
            if (set) {
                System.out.println(
                        "Adding feature repository:"
                                + repositoryURI
                                + " to profile:"
                                + profile.getId());
            } else if (delete) {
                System.out.println(
                        "Deleting feature repository:"
                                + repositoryURI
                                + " from profile:"
                                + profile.getId());
            }
            updateConfig(
                    conf,
                    ProfileConstants.REPOSITORY_PREFIX + repositoryURI.replace('/', '_'),
                    repositoryURI,
                    set,
                    delete);
        }
    ...
    private void handleLibraries(
        ...
        for (String lib : libs) {
            if (set) {
                System.out.println(
                        "Adding " + libType + ":" + lib + " to profile:" + profile.getId());
            } else if (delete) {
                System.out.println(
                        "Deleting " + libType + ":" + lib + " from profile:" + profile.getId());
            }
    ...
    private void handleBundles(ProfileBuilder builder, String[] bundles, Profile profile) {
        ...
        for (String bundle : bundles) {
            if (set) {
                ...
                System.out.println(
                        "Deleting bundle:" + bundle + " from profile:" + profile.getId());
            }
            updateConfig(
                    conf,
                    ProfileConstants.BUNDLE_PREFIX + bundle.replace('/', '_'),
                    bundle,
                    set,
                    delete);
        }
    ...
    private void handleOverrides(ProfileBuilder builder, String[] overrides, Profile profile) {
        ...
        for (String override : overrides) {
            if (set) {
                System.out.println(
                        "Adding override:" + override + " to profile:" + profile.getId());
            } else if (delete) {
                System.out.println(
                        "Deleting override:" + override + " from profile:" + profile.getId());
            }
            updateConfig(
                    conf,
                    ProfileConstants.OVERRIDE_PREFIX + override.replace('/', '_'),
                    override,
                    set,
                    delete);
        }
    ...
    private boolean handlePid(ProfileBuilder builder, String[] pidProperties, Profile profile) {
        ...
        for (String pidProperty : pidProperties) {
            ...
            if (pidProperties.length == 1 && importPid) {
                System.out.println(
                        "Importing pid:" + currentPid + " to profile:" + profile.getId());
            ...
            if (configMap.isEmpty() && set) {
                ...
                editInline = true;
                System.out.println(
                        "Deleting pid:" + currentPid + " from profile:" + profile.getId());
                ...
                for (Map.Entry<String, String> configEntries : configMap.entrySet()) {
                    ...
                    if (value == null && delete) {
                        ...
                        updatedDelimitedList(
                                conf, key, value, delimiter, set, delete, append, remove);
                    }
                }
                editInline = true;
                builder.addConfiguration(currentPid, conf);
    ...
    private void handleSystemProperties(
            ProfileBuilder builder, String[] systemProperties, Profile profile) {
        Map<String, Object> conf = getConfigurationFromBuilder(builder, Profile.INTERNAL_PID);
        for (String systemProperty : systemProperties) {
            ...
            for (Map.Entry<String, String> configEntries : configMap.entrySet()) {
                ...
                if (append) {
                    System.out.println(
                            "Appending value:"
                                    ...
                                    + " key:"
                                    + key
                                    + " from system properties and profile:"
                                    + profile.getId());
                } else if (delete) {
                    System.out.println(
                            "Deleting key:"
                ...
                updatedDelimitedList(
                        conf,
                        ProfileConstants.SYSTEM_PREFIX + key,
                        value,
                        delimiter,
                        set,
                        delete,
    ...
    private void handleConfigProperties(
            ProfileBuilder builder, String[] configProperties, Profile profile) {
        Map<String, Object> conf = getConfigurationFromBuilder(builder, Profile.INTERNAL_PID);
        for (String configProperty : configProperties) {
            ...
            for (Map.Entry<String, String> configEntries : configMap.entrySet()) {
                ...
                if (append) {
                    System.out.println(
                            "Appending value:"
                                    ...
                                    + " key:"
                                    + key
                                    + " from config properties and profile:"
                                    + profile.getId());
                } else if (delete) {
                    System.out.println(
                            "Deleting key:"
                ...
                updatedDelimitedList(
                        conf,
                        ProfileConstants.CONFIG_PREFIX + key,
                        value,
                        delimiter,
                        set,
                        delete,
    ...
    public void updatedDelimitedList(
    ...
    private void updateConfig(
            Map<String, Object> map, String key, Object value, boolean set, boolean delete) {
        if (set) {
            map.put(key, value);
        } else if (delete) {
            map.remove(key);
        ...
        try {
            ...
            if (configuration != null && configuration.length > 0) {
                ...
                while (keyEnumeration.hasMoreElements()) {
                    ...
                    // file.install.filename needs to be skipped as it specific to the current
                    // container.
    ...
    /**
     * Extracts Key value pairs from a delimited string of key value pairs. Note: The value may
     * contain commas.
     */
    ...
    /**
     * private jline.Terminal getTerminal() throws Exception { try { return (jline.Terminal)
     * terminal.getClass().getMethod("getTerminal").invoke(terminal); } catch (Throwable t) { return
     * new TerminalSupport(true) { @Override public int getWidth() { return terminal.getWidth();
     * } @Override public int getHeight() { return terminal.getHeight(); } }; } }
     *
     * <p>static class DatastoreContentManager implements ContentManager {
     *
     * <p>private static final Charset UTF_8 = Charset.forName("UTF-8");
     *
     * <p>private final ProfileService profileService;
     *
     * <p>public DatastoreContentManager(ProfileService profileService) { this.profileService =
     * profileService; } @Override public String load(String location) throws IOException { try {
     * String[] parts = location.trim().split(" "); if (parts.length < 3) { throw new
     * IllegalArgumentException("Invalid location:" + location); } String profileId = parts[0];
     * String resource = parts[1]; Profile profile = profileService.getRequiredProfile(profileId);
     * String data = new String(profile.getFileConfiguration(resource)); return data; } catch
     * (Exception e) { throw new IOException("Failed to read data from zookeeper.", e); }
     * } @Override public boolean save(String content, String location) { try { String[] parts =
     * location.trim().split(" "); if (parts.length < 3) { throw new
     * IllegalArgumentException("Invalid location:" + location); } String profileId = parts[0];
     * String resource = parts[1]; Profile profile = profileService.getRequiredProfile(profileId);
     * ProfileBuilder builder = ProfileBuilder.Factory.createFrom(profile);
     * builder.addFileConfiguration(resource, content.getBytes());
     * profileService.updateProfile(builder.getProfile()); } catch (Exception e) { return false; }
     * return true; } @Override public boolean save(String content, Charset charset, String
     * location) { return save(content, location); } @Override public Charset detectCharset(String
     * location) { return UTF_8; } }
     */
