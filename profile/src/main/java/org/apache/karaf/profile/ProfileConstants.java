...
public interface ProfileConstants {
    ...
    /**
     * The attribute key for the <em>overlay</em> flag meaning the parents are already
     * included/merged in the profile.
     */
     ...
     *
     * <p>This value can appear as the value of a key in a configuration or as a key itself. If used
     * as a key, the whole configuration is flagged as deleted from its parent when computing the
     * overlay.
     */
    ...
    /**
     * The prefix for attributes that are targeted for <code>${karaf.etc}/config.properties</code>
     * file
     */
    String CONFIG_PREFIX = "config.";

    /**
     * The prefix for attributes that are targeted for <code>${karaf.etc}/system.properties</code>
     * file
     */
    ...
    /**
     * The prefix for attributes that specify feature names (<code>name[/version]</code>) to
     * install/use
     */
    ...
    /**
     * The prefix for attributes that specify additional libraries to add to <code>${karaf.home}/lib
     * </code>. These are native libraries only. JARs that should be available in app classpath
     * should go to <code>${karaf.home}/lib/boot</code> and use {@link #BOOT_PREFIX}.
     */
    String LIB_PREFIX = "library.";

    /**
     * The prefix for attributes that specify additional endorsed libraries to add to <code>
     * ${karaf.home}/lib/endorsed</code>
     */
    String ENDORSED_PREFIX = "endorsed.";

    /**
     * The prefix for attributes that specify additional extension libraries to add to <code>
     * ${karaf.home}/lib/ext</code>
     */
    String EXT_PREFIX = "ext.";

    /**
     * The prefix for attributes that specify additional endorsed libraries to add to <code>
     * ${karaf.home}/lib/boot</code>
     */
    String BOOT_PREFIX = "boot.";

    /**
     * The prefix for attributes that specify bundle overrides (see {@link
     * org.apache.karaf.features.internal.service.Overrides}). In version 4.2 it's better to use
