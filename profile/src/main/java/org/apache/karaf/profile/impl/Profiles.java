...
public final class Profiles {
    ...
    /**
     * Loads profiles from given directory path. A profile is represented as directory with <code>
     * .profile</code> extension. Subdirectories constitute part of {@link Profile#getId} -
     * directory separators are changed to <code>-</code>. For example, profile contained in
     * directory <code>
     * mq/broker/standalone.profile</code> will have id = <code>mq-broker-standalone</code>.
     */
                    ...
                    public FileVisitResult preVisitDirectory(Path dir, BasicFileAttributes attrs) {
                        ...
                        if (fileName != null
                                && (fileName.toString().endsWith(PROFILE_FOLDER_SUFFIX)
                                        || fileName.toString()
                                                .endsWith(PROFILE_FOLDER_SUFFIX + "/"))) {
                            ...
                            profileId =
                                    profileId.substring(
                                            0, profileId.length() - PROFILE_FOLDER_SUFFIX.length());
    ...
    public static void deleteProfile(Path root, String id) throws IOException {
        Path path =
                root.resolve(
                        id.replaceAll("-", root.getFileSystem().getSeparator())
    ...
    /**
     * Writes given {@link Profile} under a path specified as <code>root</code>. Directory name to
     * store a profile is derived from {@link Profile#getId()}
     *
     * @param root
     * @param profile
     * @throws IOException
    ...
    public static void writeProfile(Path root, Profile profile) throws IOException {
        Path path =
                root.resolve(
                        profile.getId().replaceAll("-", root.getFileSystem().getSeparator())
                                + PROFILE_FOLDER_SUFFIX);
        Files.createDirectories(path);
        for (Map.Entry<String, byte[]> entry : profile.getFileConfigurations().entrySet()) {
            Files.write(
                    path.resolve(entry.getKey()), entry.getValue(), StandardOpenOption.CREATE_NEW);
        }
    }

    /**
     * Gets an <em>overlay</em> profile for given <code>profile</code>, where passed in map of
     * additional profiles is searched for possible parent profiles of given <code>profile</code>.
     *
     * @param profile
     * @param profiles
     * @return
     ...
     * additional profiles is searched for possible parent profiles of given <code>profile</code>.
     *
     * <p><code>environment</code> may be used to select different <em>variants</em> of profile
     * configuration files. For example, if <code>environment</code> is specified, configuration for
     * <code>my.pid</code> PID will be read from <code>my.pid.cfg#&lt;environment&gt;</code>.
     *
     * @param profile
     * @param profiles
     * @param environment
     * @return
     */
    public static Profile getOverlay(
            Profile profile, Map<String, Profile> profiles, String environment) {
    ...
    /**
     * Gets an <code>effective</code> profile with single property placeholder resolver for <code>
     * ${profile:xxx}</code> placeholders and with <code>finalSubstitution</code> set to <code>true
     * </code>.
     *
     * @param profile
     * @return
    ...
    /**
     * Gets an <code>effective</code> profile with <code>finalSubstitution</code> set to <code>true
     * </code>.
     *
     * @param profile
     * @param resolvers
     * @return
     */
    public static Profile getEffective(
            final Profile profile, final Collection<PlaceholderResolver> resolvers) {
        return getEffective(profile, resolvers, true);
    }

    /**
     * Gets an <em>effective</em> profile for given <code>profile</code>. Effective profile has all
     * property placeholders resolved. When <code>finalSubstitution</code> is <code>true</code>,
     * placeholders that can't be resolved are replaced with empty strings. When it's <code>false
     * </code>, placeholders are left unchanged.
     *
     * @param profile
     * @param resolvers
     * @param finalSubstitution
            ...
            if (entry.getKey().endsWith(Profile.PROPERTIES_SUFFIX)) {
                try {
                    String key =
                            entry.getKey()
                                    .substring(
                                            0,
                                            entry.getKey().length()
                                                    - Profile.PROPERTIES_SUFFIX.length());
                    ...
                    throw new IllegalArgumentException(
                            "Can not load properties for " + entry.getKey());
                }
            }
        }
    ...
    /**
     * Helper internal class to configure {@link ProfileBuilder} used to create an <em>overlay</em>
     * profile.
     *
     * <p>There are strict rules built on a concept of profiles being <em>containers of file
     * configurations</em>. Each profile may contain files with the same name. Profiles may be set
     * in multi-parent - child relationship. Such graph of profiles is searched in depth-first
     * fashion, while child (being a root of the graph) has highest priority.
     *
     * <p>Files from higher-priority profile override files from parent profiles. Special case are
     * PID files (with {@link Profile#PROPERTIES_SUFFIX} extension). These files are not simply
     * taken from child profiles. Child profiles may have own version of given PID configuration
     * file, but these files are overwritten at property level.
     *
     * <p>For example, if parent profile specifies:
     *
     * <pre>
     * property1 = v1
     * property2 = v2
     ...
     * </pre>
     *
     * an <em>overlay</em> profile for child profile uses:
     *
     * <pre>
     * property1 = v1a
     * property2 = v2
    ...
    private static class OverlayOptionsProvider {
        ...
        private OverlayOptionsProvider(
                Map<String, Profile> profiles, Profile self, String environment) {
