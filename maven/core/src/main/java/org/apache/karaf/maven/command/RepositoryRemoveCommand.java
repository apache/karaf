...
public class RepositoryRemoveCommand extends RepositoryEditCommandSupport {
    ...
    protected void edit(
        ...
        if (force
                || confirm(
                        String.format(
                                "Are you sure to remove repository with ID \"%s\" for URL %s? (y/N) ",
                                id, first.get().getURL()))) {
            if (!defaultRepository && first.get().getFrom() == MavenRepositoryURL.FROM.SETTINGS) {
                ...
                for (Profile profile : mavenSettings.getProfiles()) {
                    if (profile.getRepositories().stream().anyMatch((r) -> id.equals(r.getId()))) {
                        ...
                        System.out.printf(
                                "Repository with ID \"%s\" was removed from profile \"%s\"\n",
                                id, profile.getId());
                        break;
                    }
                }
                ...
                // if there are credentials for this repository, we have to remove them from
                // settings.xml
