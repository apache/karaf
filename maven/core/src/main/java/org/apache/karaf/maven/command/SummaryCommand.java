...
public class SummaryCommand extends MavenSecuritySupport {
    ...
    @Option(
            name = "-p",
            aliases = {"--property-ids"},
            description = "Use PID property identifiers instead of their names",
            required = false,
            multiValued = false)
    boolean propertyIds;
    ...
    @Option(
            name = "-s",
            aliases = {"--source"},
            description = "Adds information about where the value is configured",
            required = false,
            multiValued = false)
    boolean source;
    ...
    @Option(
            name = "-d",
            aliases = {"--description"},
            description = "Adds description of Maven configuration options",
            required = false,
            multiValued = false)
    boolean description;
    ...
    protected void doAction(String prefix, Dictionary<String, Object> config) throws Exception {
        ...
        if (showPasswords) {
            addRow(
                    table,
                    propertyIds ? "<master>" : "Master password",
                    new SourceAnd<String>(securitySettings.source, masterPassword),
        ...
        SourceAnd<String> updatePolicy =
                updatePolicy((String) config.get(prefix + PROPERTY_GLOBAL_UPDATE_POLICY));
        addRow(
                table,
                propertyIds ? PROPERTY_GLOBAL_UPDATE_POLICY : "Global update policy",
                updatePolicy,
        ...
        SourceAnd<String> checksumPolicy =
                checksumPolicy((String) config.get(prefix + PROPERTY_GLOBAL_CHECKSUM_POLICY));
        addRow(
                table,
                propertyIds ? PROPERTY_GLOBAL_CHECKSUM_POLICY : "Global checksum policy",
                checksumPolicy,
        ...
        String sourceInfo =
                String.format(PATTERN_PID_PROPERTY, PID, prefix + PROPERTY_UPDATE_RELEASES);
        ...
        addRow(
                table,
                propertyIds ? PROPERTY_UPDATE_RELEASES : "Update releases",
                new SourceAnd<Boolean>(sourceInfo, updateReleases),
        ...
        String requireConfigAdminProperty =
                context.getProperty(prefix + REQUIRE_CONFIG_ADMIN_CONFIG);
        ...
        addRow(
                table,
                propertyIds ? REQUIRE_CONFIG_ADMIN_CONFIG : "Require Config Admin",
                new SourceAnd<Boolean>(sourceInfo, requireConfigAdmin),
                "Whether MavenResolver service is registered ONLY with proper "
                        + PID
                        + " PID configuration");

        // see org.ops4j.pax.url.mvn.internal.config.MavenConfigurationImpl.buildSettings()
        String useFallbackRepositoriesProperty =
                (String) config.get(prefix + PROPERTY_USE_FALLBACK_REPOSITORIES);
        ...
        if (useFallbackRepositoriesProperty != null) {
            sourceInfo =
                    String.format(
                            PATTERN_PID_PROPERTY, PID, prefix + PROPERTY_USE_FALLBACK_REPOSITORIES);
        }
        addRow(
                table,
                propertyIds ? PROPERTY_USE_FALLBACK_REPOSITORIES : "Use fallback repository",
                new SourceAnd<Boolean>(sourceInfo, useFallbackRepositories),
        ...
        addRow(
                table,
                propertyIds ? PROPERTY_OFFLINE : "Offline mode",
                new SourceAnd<Boolean>(sourceInfo, offline),
        ...
        if (certificateCheckProperty != null) {
            sourceInfo =
                    String.format(PATTERN_PID_PROPERTY, PID, prefix + PROPERTY_CERTIFICATE_CHECK);
        }
        addRow(
                table,
                propertyIds ? PROPERTY_CERTIFICATE_CHECK : "SSL/TLS certificate check",
                new SourceAnd<Boolean>(sourceInfo, certificateCheck),
        ...
        for (MavenRepositoryURL url : remoteRepositories) {
            addRow(
                    table,
                    first ? (propertyIds ? PROPERTY_REPOSITORIES : "Remote repositories") : "",
                    new SourceAnd<String>(url.getFrom().getSource(), url.getURL().toString()),
                    first
                            ? "Remote repositories where artifacts are being resolved if not found locally"
                            : "");
            first = false;
        }
        ...
        for (MavenRepositoryURL url : defaultRepositories) {
            addRow(
                    table,
                    first
                            ? (propertyIds ? PROPERTY_DEFAULT_REPOSITORIES : "Default repositories")
                            : "",
                    new SourceAnd<String>(url.getFrom().getSource(), url.getURL().toString()),
                    first
                            ? "Repositories where artifacts are looked up before trying remote resolution"
                            : "");
            first = false;
        }
        ...
        if (mavenSettings != null && mavenSettings.getProxies() != null) {
            ...
            for (Proxy proxy : mavenSettings.getProxies()) {
                ...
                addRow(
                        table,
                        first ? (propertyIds ? "<proxies>" : "HTTP proxies") : "",
                        new SourceAnd<String>(MavenRepositoryURL.FROM.SETTINGS.getSource(), value),
    ...
    private <T> void addRow(
            ShellTable table, String label, SourceAnd<T> value, String descriptionText) {
