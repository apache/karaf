...
public class RepositoryAddCommand extends RepositoryEditCommandSupport {
    ...
    protected void edit(
        ...
        if (idx > pidRepos.length) {
            ...
            System.err.printf(
                    "List of %s repositories has %d elements. Can't insert at position %s.\n",
                    (defaultRepository ? "default" : "remote"), pidRepos.length, id);
            return;
        }

        Optional<MavenRepositoryURL> first =
                Arrays.stream(allRepos).filter((repo) -> id.equals(repo.getId())).findAny();
        if (first.isPresent()) {
            System.err.printf(
                    "Repository with ID \"%s\" is already configured for URL %s\n",
                    id, first.get().getURL());
            return;
        }
        ...
        sb.append(ServiceConstants.SEPARATOR_OPTIONS + ServiceConstants.OPTION_UPDATE + "=")
                .append(updatePolicy);
        sb.append(ServiceConstants.SEPARATOR_OPTIONS + ServiceConstants.OPTION_CHECKSUM + "=")
                .append(checksumPolicy);
