 ...
 *
 * <p>This code is designed to implement the JAXB 1.0 spec pluggability feature
 *
 * @author
 *     <ul>
...
class ContextFinder {
    ...
    /**
     * If the {@link InvocationTargetException} wraps an exception that shouldn't be wrapped, throw
     ...
     *
     * <p>For example, (targetType)originalType
     *
    ...
    private static JAXBException handleClassCastException(Class originalType, Class targetType) {
        ...
        ClassLoader cl =
                originalType.getClassLoader() != null
    ...
    static JAXBContext newInstance(
        ...
        try {
            ...
            try {
                Method m =
                        spiClass.getMethod(
                                "createContext", String.class, ClassLoader.class, Map.class);
                // Throw an early exception instead of having an exception thrown in the
                // createContext
                // method
            ...
            if (context == null) {
                ...
                // it is an error for an implementation not to have both forms of the createContext
                // method.
            ...
            throw new JAXBException(
                    Messages.format(Messages.COULD_NOT_INSTANTIATE, className, e), e);
            ...
            throw new JAXBException(
                    Messages.format(Messages.COULD_NOT_INSTANTIATE, className, x), x);
        }
    }
    ...
    static JAXBContext find(
            String factoryId, String contextPath, ClassLoader classLoader, Map properties)
            throws JAXBException {

        // TODO: do we want/need another layer of searching in $java.home/lib/jaxb.properties like
        // JAXP?
        ...
        while (packages.hasMoreTokens()) {
            ...
            if (props != null) {
                if (props.containsKey(factoryId)) {
                    ...
                    throw new JAXBException(
                            Messages.format(Messages.MISSING_PROPERTY, packageName, factoryId));
                }
            }
        }
        ...
        try {
            final StringBuilder resource =
                    new StringBuilder().append("META-INF/services/").append(jaxbContextFQCN);
    ...
    static JAXBContext find(Class[] classes, Map properties) throws JAXBException {
        ...
        for (final Class c : classes) {
            ...
            if (pkg == null)
                continue; // this is possible for primitives, arrays, and classes that are loaded by
            // poorly
            // implemented ClassLoaders
            String packageName = pkg.getName().replace('.', '/');

            // TODO: do we want to optimize away searching the same package?  org.Foo, org.Bar,
            // com.Baz
            //       classes from the same package might come from different class loades, so it
            // might be
            // a bad idea
            ...
            if (props == null) {
                ...
                if (props.containsKey(JAXB_CONTEXT_FACTORY)) {
                    ...
                    throw new JAXBException(
                            Messages.format(
                                    Messages.MISSING_PROPERTY, packageName, JAXB_CONTEXT_FACTORY));
                }
            }
        }
        ...
        try {
            final String resource =
                    new StringBuilder("META-INF/services/").append(jaxbContextFQCN).toString();
            ClassLoader classLoader = Thread.currentThread().getContextClassLoader();
            URL resourceURL;
            if (classLoader == null) resourceURL = ClassLoader.getSystemResource(resource);
            else resourceURL = classLoader.getResource(resource);
    ...
    private static Properties loadJAXBProperties(ClassLoader classLoader, String propFileName)
        ...
        try {
            URL url;
            if (classLoader == null) url = ClassLoader.getSystemResource(propFileName);
            else url = classLoader.getResource(propFileName);
    ...
    /**
     * Search the given ClassLoader for an instance of the specified class and return a string
     * representation of the URL that points to the resource.
     *
     * @param clazz The class to search for
     * @param loader The ClassLoader to search. If this parameter is null, then the system class
     *     loader will be searched
     ...
     *
     * <p>Convenience method for {@link #which(Class, ClassLoader)}.
     *
     * <p>Equivalent to calling: which(clazz, clazz.getClassLoader())
     *
     * @param clazz The class to search for
     * @return the URL for the class or null if it wasn't found
     */
     ...
     * META-INF/services/javax.xml.bind.JAXBContext because if it has, it will take precedence over
     * any file that applications have in their jar files.
     *
     * <p>When the user bundles his own JAXB implementation, we'd like to use it, and we want the
     * platform default to be used only when there's no other JAXB provider.
     *
     * <p>For this reason, we have to hard-code the class name into the API.
     */
