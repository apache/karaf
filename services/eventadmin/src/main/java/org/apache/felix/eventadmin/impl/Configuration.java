...
/**
 * The <code>Configuration</code> class encapsules the configuration for the event admin.
 *
 * <p>The service knows about the following properties which are read at bundle startup:
 *
 * <p><tt>org.apache.felix.eventadmin.ThreadPoolSize</tt> - The size of the thread pool.
 *
 * <p>The default value is 10. Increase in case of a large amount of synchronous events where the
 * <tt>EventHandler</tt> services in turn send new synchronous events in the event dispatching
 ...
 *
 * <p><tt>org.apache.felix.eventadmin.Timeout</tt> - The black-listing timeout in milliseconds
 *
 * <p>The default value is 5000. Increase or decrease at own discretion. A value of less then 100
 * turns timeouts off. Any other value is the time in milliseconds granted to each
 * <tt>EventHandler</tt> before it gets blacklisted.
 *
 * <p><tt>org.apache.felix.eventadmin.RequireTopic</tt> - Are <tt>EventHandler</tt> required to be
 * registered with a topic?
 *
 * <p>The default is <tt>true</tt>. The specification says that <tt>EventHandler</tt> must register
 * with a list of topics they are interested in. Setting this value to <tt>false</tt> will enable
 * that handlers without a topic are receiving all events (i.e., they are treated the same as with a
 * topic=*).
 *
 * <p><tt>org.apache.felix.eventadmin.IgnoreTimeout</tt> - Configure <tt>EventHandler</tt>s to be
 * called without a timeout.
 *
 * <p>If a timeout is configured by default all event handlers are called using the timeout. For
 * performance optimization it is possible to configure event handlers where the timeout handling is
 * not used - this reduces the thread usage from the thread pools as the timout handling requires an
 * additional thread to call the event handler. However, the application should work without this
 * configuration property. It is a pure optimization. The value is a list of string (separated by
 * comma). If the string ends with a dot, all handlers in exactly this package are ignored. If the
 * string ends with a star, all handlers in this package and all subpackages are ignored. If the
 * string neither ends with a dot nor with a start, this is assumed to define an exact class name.
 *
 * <p>These properties are read at startup and serve as a default configuration. If a configuration
 * admin is configured, the event admin can be configured through the config admin.
 *
...
public class Configuration {
    ...
    static final String PROP_ASYNC_TO_SYNC_THREAD_RATIO =
    ...
    public Configuration(BundleContext bundleContext) {
        ...
        try {
            ...
            if (service != null) {
                ...
                m_managedServiceReg =
                        m_bundleContext.registerService(interfaceNames, service, props);
            }
        } catch (Throwable t) {
            // don't care
        }
        ...
        if (config == null) {
            ...
            m_threadPoolSize =
                    getIntProperty(
                            ...
                            m_bundleContext.getProperty(PROP_THREAD_POOL_SIZE),
                            20,
                            2);
            ...
            m_asyncToSyncThreadRatio =
                    getDoubleProperty(
                            PROP_ASYNC_TO_SYNC_THREAD_RATIO,
                            m_bundleContext.getProperty(PROP_ASYNC_TO_SYNC_THREAD_RATIO),
                            0.5,
                            0.0);
            ...
            m_timeout =
                    getIntProperty(
                            PROP_TIMEOUT,
                            m_bundleContext.getProperty(PROP_TIMEOUT),
                            5000,
                            Integer.MIN_VALUE);
            ...
            m_requireTopic =
                    getBooleanProperty(m_bundleContext.getProperty(PROP_REQUIRE_TOPIC), true);
            ...
            m_addTimestamp =
                    getBooleanProperty(m_bundleContext.getProperty(PROP_ADD_TIMESTAMP), false);
            m_addSubject = getBooleanProperty(m_bundleContext.getProperty(PROP_ADD_SUBJECT), false);
        } else {
            m_threadPoolSize =
                    getIntProperty(PROP_THREAD_POOL_SIZE, config.get(PROP_THREAD_POOL_SIZE), 20, 2);
            m_asyncToSyncThreadRatio =
                    getDoubleProperty(
                            PROP_ASYNC_TO_SYNC_THREAD_RATIO,
                            m_bundleContext.getProperty(PROP_ASYNC_TO_SYNC_THREAD_RATIO),
                            ...
                            0.0);
            m_timeout =
                    getIntProperty(PROP_TIMEOUT, config.get(PROP_TIMEOUT), 5000, Integer.MIN_VALUE);
            ...
            if (value instanceof String) {
                ...
                LogWrapper.getLogger()
                        .log(
                                ...
                                "Value for property: "
                                        + PROP_IGNORE_TIMEOUT
                                        + " is neither a string nor a string array - Using default");
            }
            m_ignoreTopics = null;
            ...
            m_logLevel =
                    getIntProperty(
                            ...
                            LogWrapper.LOG_WARNING, // default log level is WARNING
                            LogWrapper.LOG_ERROR);
            m_addTimestamp = getBooleanProperty(config.get(PROP_ADD_TIMESTAMP), false);
            m_addSubject = getBooleanProperty(config.get(PROP_ADD_SUBJECT), false);
        }
        // a timeout less or equals to 100 means : disable timeout
        if (m_timeout <= 100) {
            m_timeout = 0;
        }
        m_asyncThreadPoolSize =
                m_threadPoolSize > 5
                        ? (int) Math.floor(m_threadPoolSize * m_asyncToSyncThreadRatio)
                        : 2;
    }

    private void startOrUpdate() {
        LogWrapper.getLogger().setLogLevel(m_logLevel);
        LogWrapper.getLogger().log(LogWrapper.LOG_DEBUG, PROP_LOG_LEVEL + "=" + m_logLevel);
        LogWrapper.getLogger()
                .log(LogWrapper.LOG_DEBUG, PROP_THREAD_POOL_SIZE + "=" + m_threadPoolSize);
        LogWrapper.getLogger()
                .log(
                        LogWrapper.LOG_DEBUG,
                        PROP_ASYNC_TO_SYNC_THREAD_RATIO + "=" + m_asyncToSyncThreadRatio);
        LogWrapper.getLogger()
                .log(LogWrapper.LOG_DEBUG, "Async Pool Size=" + m_asyncThreadPoolSize);
        LogWrapper.getLogger().log(LogWrapper.LOG_DEBUG, PROP_TIMEOUT + "=" + m_timeout);
        LogWrapper.getLogger().log(LogWrapper.LOG_DEBUG, PROP_REQUIRE_TOPIC + "=" + m_requireTopic);
        ...
        if (m_admin == null) {
            ...
            m_registration =
                    m_bundleContext.registerService(
                            EventAdmin.class.getName(), new SecureEventAdminFactory(m_admin), null);
        } else {
            m_admin.update(
                    m_timeout,
                    m_ignoreTimeout,
    ...
    /**
     * Called upon stopping the bundle. This will block until all pending events are delivered. An
     * IllegalStateException will be thrown on new events starting with the begin of this method.
     * However, it might take some time until we settle down which is somewhat cumbersome given that
     * the spec asks for return in a timely manner.
     */
    public void destroy() {
        synchronized (this) {
            if (m_adapters != null) {
    ...
    private Object tryToCreateMetaTypeProvider(final Object managedService) {
        try {
            return new MetaTypeProviderImpl(
    ...
    /**
     * Returns either the parsed int from the value of the property if it is set and not less then
     * the min value or the default. Additionally, a warning is generated in case the value is
     * erroneous (i.e., can not be parsed as an int or is less then the min value).
     */
    private int getIntProperty(
            final String key, final Object value, final int defaultValue, final int min) {
        if (null != value) {
            final int result;
            if (value instanceof Integer) {
                ...
                try {
                    ...
                    LogWrapper.getLogger()
                            .log(
                                    LogWrapper.LOG_WARNING,
                                    "Unable to parse property: " + key + " - Using default",
                                    e);
                    return defaultValue;
                }
            }
    ...
    /**
     * Returns either the parsed double from the value of the property if it is set and not less
     * then the min value or the default. Additionally, a warning is generated in case the value is
     * erroneous (i.e., can not be parsed as an double or is less then the min value).
     */
    private double getDoubleProperty(
            final String key, final Object value, final double defaultValue, final double min) {
        if (null != value) {
            final double result;
    ...
    /**
     * Returns true if the value of the property is set and is either 1, true, or yes Returns false
     * if the value of the property is set and is either 0, false, or no Returns the defaultValue
     * otherwise
     */
    private boolean getBooleanProperty(final Object obj, final boolean defaultValue) {
        if (null != obj) {
            ...
            if (0 < value.length()
                    && ("0".equals(value) || "false".equals(value) || "no".equals(value))) {
                return false;
            }

            if (0 < value.length()
                    && ("1".equals(value) || "true".equals(value) || "yes".equals(value))) {
                return true;
            }
        }
