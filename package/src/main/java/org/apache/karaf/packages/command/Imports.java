...
@Command(
        scope = "package",
        name = "imports",
        description = "Lists imported packages and the bundles that import them")
@Service
public class Imports implements Action {

    @Option(
            name = "--filter",
            description = "Only show package instead of full filter",
            required = false,
            multiValued = false)
    boolean showFilter;
    ...
    @Option(
            name = "-b",
            description = "Only show imports of the given bundle id",
            required = false,
            multiValued = false)
    Integer bundleId;
        ...
        for (PackageRequirement req : imports) {
            if (matchesFilter(req)) {
                ...
                if (showFilter) {
                    ...
                    row.addContent(
                            getOptional(req), bundle.getBundleId(), bundle.getSymbolicName());
                } else {
